...
class Dual_Image_dataset(data.Dataset):
    ...
    def __init__(
        ...
        background_image_names = [
            ...
            if os.path.isfile(os.path.join(self.background_dir, item))
        ]
        ...
        if GT_mask_dataset_path == None:
            ...
            GT_mask_image_names = [
                item
                for item in os.listdir(self.GT_mask_dir)
                if os.path.isfile(os.path.join(self.GT_mask_dir, item))
            ]
        ...
        assert (
            len(background_image_names) == self.dataset_length
        ), f"error input dataset has length {self.dataset_length} but background dataset length is {len(background_image_names)}"

        if self.GT_available:
            assert (
                len(GT_mask_image_names) == self.dataset_length
            ), f"error input dataset has length {self.dataset_length} but GT mask dataset length is {len(GT_mask_image_names)}"

        print(f"dataset length {len(background_image_names)}")
        ...
        print(
            f"finput dataset has {self.input_nc} channels, target dataset has {self.target_nc} channels"
        )
        print(
            f"dataset initialized  w = {self.image_width},h = {self.image_height} number of frames {self.dataset_length}"
        )
    ...
    def __getitem__(self, idx):
        ...
        input_opencv_image = cv2.imread(
            input_image_path, cv2.IMREAD_UNCHANGED
        )  # 0-255 HWC BGR
        ...
        input_tensor_image = (
            torch.from_numpy(input_np_image).type(torch.FloatTensor) / 255
        )  # RGBA

        background_image_path = os.path.join(
            self.background_dir, self.background_image_names[idx]
        )
        background_opencv_image = cv2.imread(
            background_image_path, cv2.IMREAD_UNCHANGED
        )  # 0-255 HWC BGR
        background_rgb_image = cv2.cvtColor(
            background_opencv_image, cv2.COLOR_BGRA2RGBA
        )
        background_np_image = np.asarray(background_rgb_image)
        background_np_image = np.transpose(
            background_np_image, (2, 0, 1)
        )  # 0-255 CHW RGB or RGBA
        background_tensor_image = (
            torch.from_numpy(background_np_image).type(torch.FloatTensor) / 255
        )

        if self.GT_available:
            GT_mask_image_path = os.path.join(
                self.GT_mask_dir, self.GT_mask_image_names[idx]
            )
            GT_mask_opencv_image = Image.open(GT_mask_image_path)
            GT_mask_tensor_image = torch.from_numpy(np.array(GT_mask_opencv_image))[
                None
            ]
        else:
        ...
        return (
            input_tensor_image,
            background_tensor_image,
            GT_mask_tensor_image,
        )  # RGB, RGBA, 2D range 0-1
...
def get_train_dataset_and_dataloader():
    ...
    train_dataset = Dual_Image_dataset(
        args.train_dataset_input_path,
        args.train_dataset_background_path,
...
def get_test_dataset_and_dataloader(batch_size=args.batch_size):
    ...
    test_dataset = Dual_Image_dataset(
        args.test_dataset_input_path,
        args.test_dataset_background_path,
