...
class Segformer_model(nn.Module):
    ...
    def __init__(self, out_channels):
        ...
        segformer_model = SegformerModel.from_pretrained(
            args.pretrained_backbone_weights_reference
        )
...
class Unet(nn.Module):
    ...
    def __init__(self, image_height, image_width, nch_in, nch_out):
        ...
        self.first_feature_map = nn.Sequential(
            nn.Conv2d(nch_in, nch[0], 3, 1, 1, bias=False),
            nn.BatchNorm2d(nch[0]),
            nn.CELU(),
        )

        self.downsample_blocks = nn.ModuleList(
            [Downsample_and_Conv(nch[i], nch[i + 1]) for i in range(nlayers)]
        )
        ...
        self.upsample_blocks = nn.ModuleList(
            [
                Conv_and_Upsample(2 * nch[i + 1], nch[i], output_paddings[i])
...
class Encoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        feature_and_attention_map_dim = (
            self.args.max_set_size + self.args.z_what_dim + 1 + self.scaling_dim
        )  # +1 is for activation

        if args.feature_map_generator_name == "Unet":
            self.feature_and_attention_map_generator = Unet(
                args.image_height, args.image_width, 3, feature_and_attention_map_dim
            )
        elif args.feature_map_generator_name == "Segformer":
            self.feature_and_attention_map_generator = Segformer_model(
                feature_and_attention_map_dim
            )
        else:
            print(
                f"feature map generator name {args.feature_map_generator_name} not recognized "
            )
            exit(0)
        print(args.device)
        self.feature_and_attention_map_generator = (
            self.feature_and_attention_map_generator.to(args.device)
        )

        test_image = torch.rand(1, 3, args.image_height, args.image_width).to(
            args.device
        )
        bs, n_channels, self.feature_map_height, self.feature_map_width = (
            self.feature_and_attention_map_generator(test_image).shape
        )
        ...
        self.position_map = nn.Parameter(
            position_map.reshape(1, 2, fh, fw), requires_grad=False
        )

        self.feature_vector_embedding = nn.Linear(
            self.args.z_what_dim + 3 + self.scaling_dim, args.transformer_dim
        )  # K,x,y,alpha, scale
        ...
        self.transformer = nn.TransformerEncoder(
            transformer_encoder_layer, num_layers=args.transformer_nlayers
        )

        self.latents_projection = nn.Linear(
            args.transformer_dim, self.args.z_what_dim + 3 + self.scaling_dim
        )  # 3 is for  x,y,  activation
    ...
    def forward(self, input_images):
        ...
        feature_and_attention_map = self.feature_and_attention_map_generator(
            input_images
        )
        ...
        attention_weights = torch.softmax(
            attention_logits.reshape(n, k, fh * fw), dim=2
        ).reshape(n, k, fh, fw)

        position_and_feature_maps = torch.cat(
            [
                self.position_map.expand(n, -1, -1, -1),
                feature_and_attention_map[:, k:, :, :],
            ],
            dim=1,
        )

        expanded_position_and_feature_maps = position_and_feature_maps.unsqueeze(
            1
        ).expand(-1, k, -1, -1, -1)

        expanded_attention_weights = attention_weights.unsqueeze(2).expand(
            -1, -1, self.args.z_what_dim + 3 + self.scaling_dim, -1, -1
        )

        position_and_feature_latents = torch.sum(
            expanded_position_and_feature_maps * expanded_attention_weights, dim=(3, 4)
        )
        ...
        updated_position_and_feature_latents = self.latents_projection(
