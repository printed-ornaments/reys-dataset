...
def setup_background_models(
    image_height, image_width, complexity=False, test_model=True
):
    ...
    torch.save(
        {
            ...
            "encoder_state_dict": netBE.state_dict(),
...
def get_trained_model(background_model_training_scenario="random_initialization"):
    ...
    model_path = "./outputs/background_output/models/trained_model.pth"
    print(f"loading saved models from {model_path}")
    checkpoint = torch.load(model_path)
    complexity = env.background_complexity
    BE, BG = setup_background_models(env.image_height, env.image_width, complexity)
