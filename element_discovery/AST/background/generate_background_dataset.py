...
def compute_background_and_mask_using_trained_model(netBE, netBG, data):
    ...
    images = {}
    print("The device is:", env.device)
    real_images = data.to(env.device).type(torch.FloatTensor)  # range 0-255 Nx3xHxW RGB
    backgrounds_with_error_predictions = netBG(
        netBE(real_images)
    )  # range 0-255 shape Nx4xHxW RGB+error prédiction
    backgrounds = backgrounds_with_error_predictions[:, 0:3, :, :]
    error_predictions = backgrounds_with_error_predictions[
        :, 3, :, :
    ]  # NHW  0-255 float
    ...
    diffs = (
        (torch.abs((real_images - backgrounds)))
        .permute(0, 2, 3, 1)
        .cpu()
        .detach()
        .numpy()
    ...
    error_predictions = (
        error_predictions.cpu().detach().numpy().astype("uint8")
    )  # NHW 0-255
    images["noise"] = error_predictions

    NWC_backgrounds_with_error_predictions = (
        backgrounds_with_error_predictions.permute(0, 2, 3, 1)
        .cpu()
        .detach()
        .numpy()
        .astype("uint8")
    ...
    masks_before_post_processing = np.zeros(
        (batch_size, env.image_height, env.image_width)
    )
    masks = np.zeros((batch_size, env.image_height, env.image_width))

    for i in range(batch_size):

        corrected_dif = np.maximum(
            0,
            l1_errors[i].astype("int16")
            - env.alpha_2 * error_predictions[i].astype("int16"),
        )

        background_illumination = (
            (torch.sum(backgrounds[i]) / (3 * env.image_height * env.image_width))
            .detach()
            .cpu()
            .numpy()
        )  # range 0-255

        mask_before_post_processing = 255 * np.greater(
            3 * corrected_dif, env.alpha_1 * background_illumination
        ).astype("uint8")

        masks_before_post_processing[i, :, :] = mask_before_post_processing

        if (
            env.image_height < 400 and env.image_width < 400
        ):  # no postprocessing on small images
            mask = mask_before_post_processing
        else:
            close_kernel = np.ones((5, 5), np.uint8)
            mask = cv2.morphologyEx(
                mask_before_post_processing, cv2.MORPH_CLOSE, close_kernel
            )
...
def generate_images(dataloader, video_paths, netBE, netBG):
    ...
    for i, data in enumerate(tqdm(dataloader)):
        ...
        images = compute_background_and_mask_using_trained_model(
            netBE, netBG, test_images
        )
        ...
        for j in range(test_images.shape[0]):
            ...
            cv2.imwrite(
                "%s/input_image_%06d.png" % (video_paths["input_images"], index),
                real_image,
            )

            background_with_error_prediction = images[
                "backgrounds_with_error_prediction"
            ][j]
            background_with_error_prediction = cv2.cvtColor(
                background_with_error_prediction, cv2.COLOR_RGBA2BGRA
            )
            cv2.imwrite(
                "%s/background_%06d.png"
                % (video_paths["backgrounds_with_error_prediction"], index),
                background_with_error_prediction,
            )

            if GT_masks is not None:
                GT_mask = (
                    GT_mask.reshape(env.image_height, env.image_width)
                    .detach()
                    .numpy()
                    .astype("uint8")
                )
            ...
            cv2.imwrite(
                "%s/background_%06d.png" % (video_paths["backgrounds"], index),
                images["backgrounds"][j],
            )
            cv2.imwrite(
                "%s/bin%06d.png" % (video_paths["masks"], index), images["masks"][j]
            )
...
def generate_images_using_trained_model():
    ...
    netBE, netBG = utils.setup_background_models(
        test_dataset.image_height, test_dataset.image_width, complexity
    )
    ...
    video_paths["backgrounds_with_error_prediction"] = os.path.join(
        results_path, "backgrounds_rgba_train"
    )
    ...
    video_paths["backgrounds_with_error_prediction"] = os.path.join(
        results_path, "backgrounds_rgba_test"
    )
    for k, dir_path in video_paths.items():
        if not os.path.isdir(dir_path):
            os.mkdir(dir_path)
