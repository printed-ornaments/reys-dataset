...
def plot_infer_render_components(input_image, obj_prob, pred_cls, pred_bbox, output_image, writer, step):
    ...
    _plot_bounding_boxes_w_c('x_hat', pred_bbox, output_image, gs[0, 1], fig, output_image_copy, step, 'output')
...
def _plot_bounding_boxes_w_c(title, bbox, original_image, gridspec, fig, images, step, mode):
    print('Entered w c module')
    ax = fig.add_subplot(gridspec)
    ax.imshow(original_image, cmap='gray', vmin=0, vmax=1)
    fin_img = transform(images[0])
    #rand_name = generate_slug()
    st = '/home_expes/cs103329/cs103329/GMAIR-pytorch-2/examples_GMAIR/'+str(step)+'_new_'+mode+'.png'
    fin_img.save(st)
    #plt.figure()
    #plt.imshow(fin_img)
    #plt.show()
    ...
    #nxm=25
    colors = [
    (255, 0, 0),   # Sunshine Yellow
    (255, 0, 0),   # Dodger Blue
    (255,0,0)
    ]
    nim = np.random.randint(15,25)
    for i in range(nim):
        xn = random.randint(1,num-1)
        x, y, w, h, c, conf = bbox[xn]
        color = random.randint(0,2)
        '''if mode == 'output':
           in_img = cv2.rectangle(fin_img,(int(x)-int(float(w)/6),int(y)),(int(x)+int(float(w)),int(y)+int(float(h))),colors[color],1)'''
        #plt.figure()
        #plt.imshow(fin_img)
        #plt.show()
