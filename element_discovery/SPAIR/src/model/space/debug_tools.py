...
# from gmair.utils.coolname import generate_slug
...
def torch2npy(t: torch.Tensor, reshape=False):
    ...
    if reshape:
        return (
            t.cpu()
            .view(cfg.batch_size, grid_size, grid_size, *shape)
            .detach()
            .squeeze()
            .numpy()
        )
...
def plot_render_components(
    obj_vec, obj_prob, depth, bounding_box, input_image, output_image, writer, step
):
    ...
    for i in range(num_rows):
        ...
        for j in range(num_cols - 7):
            ...
            alpha_j = obj_vec[
                ...
                :,
                :,
                num_color_channels : num_color_channels + 1,
            ]
            obj_j = obj_j * alpha_j
...
def plot_infer_render_components(
    input_image, obj_prob, pred_cls, pred_bbox, output_image, step
):
    """Plots each component prior to rendering"""
    input_image_copy = input_image
    output_image_copy = output_image

    input_image = input_image[0, ...].permute(1, 2, 0).cpu().detach().squeeze().numpy()
    output_image = (
        output_image[0, ...].permute(1, 2, 0).cpu().detach().squeeze().numpy()
    )
    ...
    # Bounding Box
    # _plot_bounding_boxes_w_c('x', gt_bbox, input_image, gs[0, 0], fig, input_image_copy, step, 'groundtruth')

    _plot_bounding_boxes_w_c(
        "x_hat",
        pred_bbox,
        output_image,
        gs[0, 1],
    ...
    for i in range(H):
        for j in range(W):
            ...
            patch = patches.Rectangle(
                [x, y], w, h, facecolor="none", edgecolor=border_color, linewidth=1
            )
            ax.add_patch(patch)
...
def _plot_bounding_boxes_w_c(
    title, bbox, original_image, gridspec, fig, images, step, mode
):
    print("Entered w c module")
    ax = fig.add_subplot(gridspec)
    ax.imshow(original_image, cmap="gray", vmin=0, vmax=1)
    fin_img = transform(images[0])
    # rand_name = generate_slug()
    st = "../results/examples_SPAGMACE/gmairx/" + str(step) + "_new_full_bbox" + ".png"
    fin_img.save(st)
    # plt.figure()
    # plt.imshow(fin_img)
    # plt.show()
    fin_img = cv2.imread(st)

    num, _ = bbox.shape
    for i in range(num):
        x, y, w, h, c, conf = bbox[i]
        if mode == "output":
            fin_img = cv2.rectangle(
                fin_img,
                (int(x) - int(float(w) / 6), int(y)),
                (int(x) + int(float(w)), int(y) + int(float(h))),
                (255, 0, 0),
                1,
            )
        # plt.figure()
        # plt.imshow(fin_img)
        # plt.show()
        aaa = np.array(fin_img)
        abc = Image.fromarray(aaa)
        abc.save(st)
...
def decoder_output_grad_hook(grad, writer, step):
    ...
    grad = (
        grad.view(cfg.batch_size, GRID_SIZE, GRID_SIZE, obj_px, obj_px, 2)
        .cpu()
        .detach()
        .numpy()
    )
